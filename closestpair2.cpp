...
float closepair(Point p1[],int n){
  ...
  float d1= closepair(p1, mid);
  float d2 = closepair(p1 + mid, n-mid);
  float d = min(d1,d2);
  int j = 0;
  Point area[n];
    for (int i = 0; i < n; i++){
...
float close(Point P[], int n){
    qsort(P, n, sizeof(int), comx);
 
    return closepair(P, n);
}

int main(){
  Point p[] ={{14, 18}, {22, 13}, {18, 2}, {20, 17}, {24, 15}, {10, 5}};
